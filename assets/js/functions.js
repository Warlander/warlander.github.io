$(document).ready(function() {
  ...
  $('a[href*="#"]:not([href="#0"])').click(function(ev) {
	...
	if ($(window).width() > 991) {
	  ...
	  $.each(appDesignTrigger, function(key, value){
		var appDesignVisual = $(value).find(".app-design__visual");
	  	new ScrollMagic.Scene({
			triggerElement: value,
			reverse: false
		}).setTween(TweenLite.from(appDesignVisual, 0.8, {x: 100, opacity: 0, ease: Power1.easeOut})).addTo(controller);
	  });

	  /* Web Design */
	  var webDesignTrigger = $('.web-design');

		$.each(webDesignTrigger, function(key, value){
			var webDesignVisual = $(value).find(".web-design__visual");
			new ScrollMagic.Scene({
				triggerElement: value,
				reverse: false
			}).setTween(TweenLite.from(webDesignVisual, 0.8, {x: -100, opacity: 0, ease: Power1.easeOut})).addTo(controller);
		});
